...
describe('Contract Invoker Flows', () => {
  ...
  const MOCK_CONTRACT_ID = StellarSdk.StrKey.encodeContract(
    validBytes as unknown as Buffer,
  );
  ...
  let mockSorobanServer: {
    getLedgerEntries: ReturnType<typeof vi.fn>;
  ...
  beforeEach(() => {
    ...
    vi.mocked(getSorobanServer).mockReturnValue(mockSorobanServer as never);
    vi.mocked(getServer).mockReturnValue(mockHorizonServer as never);
    vi.mocked(isValidContractId).mockImplementation((id) => id === MOCK_CONTRACT_ID);
    vi.mocked(isValidPublicKey).mockImplementation((id) => id === MOCK_PUBKEY);
  });
  ...
  describe('invokeContractFunction (Validation & Invocation)', () => {
    it('throws error for invalid contract ID', async () => {
      vi.mocked(isValidContractId).mockReturnValueOnce(false);
    ...
    it('throws error for unsupported argument types', async () => {
      await expect(invokeContractFunction({
        contractId: MOCK_CONTRACT_ID,
        functionName: 'init',
        args: [{ type: 'float', value: '1.5' } as unknown as ContractInvokeArg],
        sourceAccount: MOCK_PUBKEY,
        secretKey: MOCK_SECRET
